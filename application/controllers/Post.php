...
class Post extends CI_Controller {
    ...
    public function index() {
        $data['title'] = 'Berita Saya | Berita 86';
    ...
    public function add() {
        $data['title'] = 'Terbitkan Berita Baru | Berita 86';
    ...
    public function delete($idpost) {
        $data['title'] = 'Hapus Berita | Berita 86';
        ...
        if($this->input->method(TRUE) == 'POST' && $this->input->post('do_delete') == 1 && $this->input->post('idpost') == $idpost) {
            ...
            $this->template->alert(
                'Hapus berita terbaru <strong>"' . $data['post']->title . '"</strong> Permanen.',
                'warning'
            );
            redirect('post');
            return;
